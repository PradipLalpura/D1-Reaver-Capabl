...
| 2026-09-21 | OAuth 2.1 AS built (stdlib only): AS + resource metadata, DCR, consent page bound to /connect tokens, code+refresh grants, PKCE S256, iss echo, opaque rotating tokens inheriting caps. Proven live end-to-end: register→consent→grant→tool→rotate. Selfcheck 88/88, evals 11/11. |
| 2026-09-21 | UI v4 (reference-grade, own brand): glass utility, staircase hero, glass terminal card, rich sitemap footer, staged entrance choreography. Borrowed language, not clones: no foreign copy/media/hotlinks. Selfcheck 88/88, evals 11/11, routes verified live. |
| 2026-09-21 | ChatGPT handshake hardening: 401s now carry `WWW-Authenticate: Bearer resource_metadata=…` + public `/.well-known/oauth-protected-resource` (proxy-aware, exempt from auth). Diagnosed from screenshots: token-in-URL → 404-class fail; No-Auth → bare-401 fail; both pre-fix behaviors confirmed live. Selfcheck 75/75, evals 11/11. |
